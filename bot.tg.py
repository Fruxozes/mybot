...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        await context.bot.send_animation(chat_id=update.effective_chat.id, animation=START_ANIMATION_URL, caption="Приветствую, ознакомься с командами бота!")
        logging.info(f"Отправлена гифка пользователю {update.message.from_user.username} ({update.message.from_user.id})")
...
async def vopros(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logging.info(f"Отправлен ответ на команду /vopros пользователю {update.message.from_user.username} ({update.message.from_user.id})")
    except Exception as e:
...
async def creator(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await  update.message.reply_text(response_text)
        logging.info(f"Отправлен ответ на команду /creator пользователю {update.message.from_user.username} ({update.message.from_user.id}")
    except Exception as e:
...
async def ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logging.info(f"Отправлен ответ на команду /ai пользователю {update.message.from_user.username} ({update.message.from_user.id}")
    except Exception as e:
...
async def skull_reaction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logging.info(f"Ответили на смайлик черепа пользователю {update.message.from_user.username} ({update.message.from_user.id})")
    except Exception as e:
